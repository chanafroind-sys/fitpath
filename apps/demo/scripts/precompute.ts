 ...
 * It used to be 15 here, silently, and every published figure was for a stud
 * partition. Each maneuver now records how thick a wall its numbers hold for.
 */
const WALL = DEFAULT_WALL_THICKNESS;
const HERO_WIDTH = 90;
...
// The product pictures, drawn from the same boxes the maneuvers were measured
// on. Six sofas shared one hand-drawn placeholder for three rounds because a
// picture somebody has to draw is a picture nobody redraws when the model
// changes; these change with it, and the build is what draws them.
const pictures = resolve(here, '../src/assets/generated');
mkdirSync(pictures, { recursive: true });
for (const item of SOFAS) {
  writeFileSync(resolve(pictures, `${item.id}.svg`), `${illustrate(item)}
`, 'utf8');
}
console.log(`drew ${SOFAS.length} illustrations into ${pictures}`);
